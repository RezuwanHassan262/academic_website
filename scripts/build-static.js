 ...
 *
 * NOTE: webpages/ is now hand-maintained and is what gets deployed — the
 * deploy runs scripts/build-site.js only. Running THIS script overwrites
 * webpages/*.html from the sources below and discards any edit made to those
 * HTML files directly, so run it only deliberately, after porting such edits
 * back into the sources.
 *
 * Sources it regenerates from:
...
// Loud on purpose: webpages/ is deployed as-is and edited by hand, so this
// script is destructive rather than routine.
console.warn('');
console.warn('  WARNING: rewriting every page in webpages/ from _pages/, _data/');
console.warn('  and the collections. Anything edited directly in those HTML files');
console.warn('  is being lost.');
console.warn('');
...
function navHtml(nav, current) {
  ...
  return items
    .map((item) => {
      ...
      // A PDF (the CV) opens in its own tab as well, so the browser PDF
      // viewer does not replace the site in the current tab.
...
<head>
    ...
    <meta name="generator" content="hand-maintained — running scripts/build-static.js regenerates and overwrites this file">
